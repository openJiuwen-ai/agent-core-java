...
class SpawnManagerCompatibilityTest {
    ...
    void spawnManagerShouldUseRunnerProcessSpawnWhenSpecRequestsProcessMode() {
        ...
        TeamRuntimeContext ctx = TeamRuntimeContext.builder()
                .teamId("spawn-manager-team")
                .sessionId("spawn-session-process")
                .memberName("worker-1")
                .role(TeamRole.MEMBER)
                .build();
    ...
    void healthCheckLoopShouldTriggerUnhealthyRecoveryWhenHandleStops() throws Exception {
        ...
        agent.getTeamBackend().spawnMember(
                "worker-1",
                "Worker One",
                AgentCard.builder().name("worker-1").description("Worker one").build()
        ).join();
        InProcessSpawnHandle stoppedHandle = InProcessSpawnHandle.builder()
                .processId("stopped-worker")
    ...
    void processHealthCheckShouldTriggerSpawnManagerRecoveryAndMonitorVisibleRestart() throws Exception {
        ...
        TeamRuntimeContext ctx = TeamRuntimeContext.builder()
                .teamId("spawn-manager-team")
                .sessionId("spawn-session-process-health")
                .memberName("worker-1")
                .role(TeamRole.MEMBER)
                .build();
        ...
        SpawnHandle firstHandle = agent.getSpawnManager().spawnTeammate(
                ctx,
                "run long enough for health failure",
    ...
    void spawnPayloadJsonRoundTripShouldPreserveMemberModelForDeepAgentConfig() throws Exception {
        ...
        agent.getTeamBackend().spawnMember(
                "pool-worker",
                "Pool Worker",
                AgentCard.builder().name("worker").description("pool-backed worker").build(),
                TeamRole.MEMBER,
                "Resume the backlog.",
                agent.allocateModel("gpt-4")
        ).join();
