...
class EventDispatcherCompatibilityTest {
    ...
    void dispatchShouldSkipWhenAgentIsNotReadyLikePythonDispatcher() {
        ...
        dispatcher.dispatch(InnerEventMessage.builder()
                .eventType(InnerEventType.USER_INPUT)
                .payload(Map.of("content", "should not configure implicitly"))
                .build());
    ...
    void transportEventShouldSkipWhenMemberNameMissingLikePythonDispatcher() {
        TeamAgent agent = new TeamAgent().configure(teamSpec(TeamRole.LEADER), com.openjiuwen.agentteams.schema.team.TeamRuntimeContext.builder()
                .teamId("dispatcher-team")
                .role(TeamRole.LEADER)
                .build());

        agent.getMessageManager().sendMessage("do not fallback to leader", TeamConstants.DEFAULT_LEADER_MEMBER_NAME, "worker-1").join();
        agent.getDispatcher().dispatch(EventMessage.builder().eventType(EventDispatcher.EVENT_MESSAGE).build());
        boolean handled = agent.getDispatcher().handleTaskClaimed(EventMessage.builder()
                .eventType(EventDispatcher.EVENT_TASK_CLAIMED)
        ...
        assertThat(agent.getMessageManager().getMessages(TeamConstants.DEFAULT_LEADER_MEMBER_NAME, true))
                ...
                .isEqualTo("do not fallback to leader");
    }
    ...
    void innerUserInputShouldRouteMentionThroughCoordinationManager() {
        ...
        agent.getDispatcher().dispatch(InnerEventMessage.builder()
                .eventType(InnerEventType.USER_INPUT)
                .payload(Map.of("content", "@worker-1 inspect mailbox"))
                .build());

        assertThat(agent.getMessageManager().getMessages("worker-1", false))
                .singleElement()
    ...
    void innerUserInputShouldFallbackToLeaderForPlainText() {
        ...
        agent.getDispatcher().dispatch(InnerEventMessage.builder()
                .eventType(InnerEventType.USER_INPUT)
                .payload(Map.of("content", "plan the release"))
                .build());
    ...
    void mailboxPollShouldReadUnreadMessagesInTimestampOrderAndMarkRead() {
        ...
        String directId = agent.getMessageManager()
                .sendMessage("direct second", TeamConstants.DEFAULT_LEADER_MEMBER_NAME, "worker-1")
                .join();
    ...
    void mailboxPollShouldDeferMessagesWhilePendingInterruptExists() {
        ...
        agent.getAgentSession().updateState(Map.of(ToolInterruptionState.INTERRUPTION_KEY, ToolInterruptionState.builder()
                .interruptedTools(List.of(ToolInterruptEntry.builder()
                        ...
                        .build()))
                .build()));
        agent.getMessageManager().sendMessage("wait for approval first", TeamConstants.DEFAULT_LEADER_MEMBER_NAME, "worker-1").join();

        List<TeamMessage> delivered = agent.getDispatcher().processUnreadMessages(TeamConstants.DEFAULT_LEADER_MEMBER_NAME);
        ...
        assertThat(agent.getMessageManager().getMessages(TeamConstants.DEFAULT_LEADER_MEMBER_NAME, true))
                .singleElement()
                .extracting(TeamMessage::getContent)
                .isEqualTo("wait for approval first");
    }

    @Test
    void teamCleanedShouldShutdownTeammateButNotLeader() {
        TeamAgent leader = TeamFactory.createAgentTeam(teamSpec(TeamRole.LEADER));
        TeamAgent teammate = new TeamAgent().configure(teamSpec(TeamRole.LEADER), com.openjiuwen.agentteams.schema.team.TeamRuntimeContext.builder()
                .teamId("dispatcher-team")
                .memberName("worker-1")
    ...
    void standbyShouldPausePollsAndMessageEventShouldResumeAndProcessMailbox() {
        ...
        agent.getMessageManager().sendMessage("wake leader", TeamConstants.DEFAULT_LEADER_MEMBER_NAME, "worker-1").join();
        agent.getDispatcher().dispatch(EventMessage.builder().eventType(EventDispatcher.EVENT_MESSAGE).build());

        assertThat(agent.getCoordinatorLoop().isPollsPaused()).isFalse();
        assertThat(agent.getLeaderInbox()).singleElement()
                .satisfies(content -> assertThat(content).contains("Received direct message", "worker-1", "wake leader"));
    ...
    void taskClaimedForSelfShouldResumePollsAndDeliverAssignment() {
        ...
        boolean handled = teammate.getDispatcher().handleTaskClaimed(EventMessage.builder()
                .eventType(EventDispatcher.EVENT_TASK_CLAIMED)
                .payload(Map.of("member_name", "worker-1", "task_id", "task-123"))
                .build());
    ...
    void taskClaimedForOtherMemberShouldBeIgnored() {
        ...
        boolean handled = teammate.getDispatcher().handleTaskClaimed(EventMessage.builder()
                .eventType(EventDispatcher.EVENT_TASK_CLAIMED)
                .payload(Map.of("member_name", "worker-2", "task_id", "task-123"))
                .build());
    ...
    void memberReadyWithStaleClaimedTaskShouldNudgeMember() {
        ...
        var stale = leader.getDispatcher().handleMemberStatusChanged(EventMessage.builder()
                .eventType(EventDispatcher.EVENT_MEMBER_STATUS_CHANGED)
                .payload(Map.of("member_name", "worker-1", "old_status", "busy", "new_status", "ready"))
                .build());

        assertThat(stale).singleElement()
                .extracting(task -> task.getTaskId(), task -> task.getTitle())
                .containsExactly("task-1", "Fix bug");
        assertThat(leader.getMessageManager().getMessages("worker-1", false))
                .singleElement()
                .satisfies(message -> {
    ...
    void memberStatusUnchangedOrWithoutStaleClaimShouldSkipNudge() {
        ...
        assertThat(leader.getDispatcher().handleMemberStatusChanged(EventMessage.builder()
                .eventType(EventDispatcher.EVENT_MEMBER_STATUS_CHANGED)
    ...
    void leaderShouldObserveMemberLifecycleEventsLikePythonDispatcher() {
        ...
        assertThat(leader.getDispatcher().handleLeaderMemberLifecycleEvent(EventMessage.builder()
                .eventType(EventDispatcher.EVENT_MEMBER_SPAWNED)
                .payload(Map.of("member_name", "worker-1"))
                .build())).isEqualTo("Member online: worker-1");
        assertThat(leader.getDispatcher().handleLeaderMemberLifecycleEvent(EventMessage.builder()
                .eventType(EventDispatcher.EVENT_MEMBER_RESTARTED)
                .payload(Map.of("member_name", "worker-1", "restart_count", 2))
                .build())).isEqualTo("Member restarted: worker-1 restart_count=2");
        assertThat(leader.getDispatcher().handleLeaderMemberLifecycleEvent(EventMessage.builder()
                .eventType(EventDispatcher.EVENT_MEMBER_EXECUTION_CHANGED)
                .payload(Map.of("member_name", "worker-1", "old_status", "running", "new_status", "completed"))
                .build())).isEqualTo("Member execution changed: worker-1 running -> completed");
        assertThat(leader.getDispatcher().handleLeaderMemberLifecycleEvent(EventMessage.builder()
                .eventType(EventDispatcher.EVENT_MEMBER_SHUTDOWN)
                .payload(Map.of("member_name", "worker-1"))
                .build())).isEqualTo("Member shutdown: worker-1");
        assertThat(leader.getDispatcher().handleLeaderMemberLifecycleEvent(EventMessage.builder()
                .eventType(EventDispatcher.EVENT_MEMBER_CANCELED)
                .payload(Map.of("member_name", "worker-1"))
                .build())).isEqualTo("Member canceled: worker-1");
    }
    ...
    void mailboxPollCanUseFollowUpForRunningAgentWhenSteerDisabled() throws Exception {
        TeamAgent leader = TeamFactory.createAgentTeam(teamSpec(TeamRole.LEADER));
        leader.getMessageManager().sendMessage("finish the prior notice", TeamConstants.DEFAULT_LEADER_MEMBER_NAME, "worker-1").join();
        setStreamingActive(leader, true);

        List<TeamMessage> delivered = leader.getDispatcher()
                .processUnreadMessages(TeamConstants.DEFAULT_LEADER_MEMBER_NAME, false);

        assertThat(delivered).singleElement()
                .extracting(TeamMessage::getContent)
                .isEqualTo("finish the prior notice");
        assertThat(leader.getLeaderInbox()).isEmpty();
        assertThat(leader.getStreamController().getPendingInputs()).isEmpty();
        assertThat(leader.getDeepAgent().getLoopController().drainFollowUp(leader.getAgentSession().getSessionId()))
                .singleElement()
                .satisfies(content -> assertThat(content).contains("Received direct message", "worker-1", "finish the prior notice"));
    ...
    void taskEventShouldSkipTaskBoardNudgeWhileRoundIsInFlight() {
        ...
        leader.getDispatcher().dispatch(EventMessage.builder()
                .eventType(EventDispatcher.EVENT_TASK_UPDATED)
                .payload(Map.of("task_id", "task-board-1"))
                .build());
    ...
    void taskBoardEventShouldNudgeIdleTeammateWithTaskList() {
        ...
        assertThat(teammate.getLeaderInbox()).singleElement()
                .satisfies(content -> assertThat(content).contains("Current teammate task list", "task-board-2", "pending"));
    }
    ...
    void leaderTaskBoardShouldPromptAllDoneOnlyOutsidePollPath() {
        ...
        assertThat(temporaryLeader.getDispatcher().nudgeIdleAgent(TeamConstants.DEFAULT_LEADER_MEMBER_NAME, true)).isEmpty();
        assertThat(temporaryLeader.getLeaderInbox()).isEmpty();

        assertThat(temporaryLeader.getDispatcher().nudgeIdleAgent(TeamConstants.DEFAULT_LEADER_MEMBER_NAME, false)).isEmpty();
        ...
        assertThat(persistentLeader.getDispatcher().nudgeIdleAgent(TeamConstants.DEFAULT_LEADER_MEMBER_NAME, false)).isEmpty();
    ...
    void pollTaskShouldNudgeStaleClaimedSelfAndThrottle() {
        ...
        assertThat(stale).singleElement()
                .extracting(task -> task.getTaskId(), task -> task.getAssignee())
    ...
    void leaderShouldAutoAckUserBoundDirectMessageEvent() {
        ...
        String messageId = leader.getMessageManager()
                .sendMessage("done for user", TeamConstants.USER_PSEUDO_MEMBER_NAME, "worker-1")
                .join();

        boolean acked = leader.getDispatcher().ackUserBoundMessage(EventMessage.builder()
                .eventType(EventDispatcher.EVENT_MESSAGE)
                .payload(Map.of(
                        "message_id", messageId,
                        "from_member_name", "worker-1",
                        "to_member_name", TeamConstants.USER_PSEUDO_MEMBER_NAME
                ))
                .build());
        ...
        assertThat(leader.getMessageManager().getMessages(TeamConstants.USER_PSEUDO_MEMBER_NAME, false))
                .singleElement()
                .extracting(message -> message.isRead())
                .isEqualTo(true);
    }
    ...
    void autoAckShouldSkipNonUserBoundMessage() {
        TeamAgent leader = TeamFactory.createAgentTeam(teamSpec(TeamRole.LEADER));
        String messageId = leader.getMessageManager()
                .sendMessage("for leader", TeamConstants.DEFAULT_LEADER_MEMBER_NAME, "worker-1")
                .join();
    ...
    void toolApprovalResultShouldResumeInterruptForTargetMember() {
        TeamAgent teammate = new TeamAgent().configure(teamSpec(TeamRole.LEADER), com.openjiuwen.agentteams.schema.team.TeamRuntimeContext.builder()
                ...
                .role(TeamRole.MEMBER)
                .build());
        teammate.getAgentSession().updateState(Map.of(ToolInterruptionState.INTERRUPTION_KEY, ToolInterruptionState.builder()
                .interruptedTools(List.of(ToolInterruptEntry.builder()
                        .request(InterruptRequest.builder().interruptId("call-1").build())
                        .build()))
                .build()));
        ...
        var input = teammate.getDispatcher().handleToolApprovalResult(EventMessage.builder()
                .eventType(EventDispatcher.EVENT_TOOL_APPROVAL_RESULT)
                .payload(Map.of(
    ...
    void memberShutdownForSelfShouldProcessUnreadMailbox() {
        ...
        assertThat(handled).isTrue();
        assertThat(teammate.getLeaderInbox()).singleElement()
                .satisfies(content -> assertThat(content).contains("Received direct message", "team_leader", "shutdown notice"));
    ...
    void teammateMemberLifecycleShouldIgnoreLeaderAndOtherMembers() {
        TeamAgent leader = TeamFactory.createAgentTeam(teamSpec(TeamRole.LEADER));
        assertThat(leader.getDispatcher().handleTeammateMemberLifecycleEvent(EventMessage.builder()
                .eventType(EventDispatcher.EVENT_MEMBER_CANCELED)
                .payload(Map.of("member_name", TeamConstants.DEFAULT_LEADER_MEMBER_NAME))
                .build())).isFalse();
        ...
        assertThat(teammate.getDispatcher().handleTeammateMemberLifecycleEvent(EventMessage.builder()
                .eventType(EventDispatcher.EVENT_MEMBER_CANCELED)
                .payload(Map.of("member_name", "worker-2"))
                .build())).isFalse();
