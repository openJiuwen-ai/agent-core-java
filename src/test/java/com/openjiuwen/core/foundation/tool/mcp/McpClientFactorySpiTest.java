...
class McpClientFactorySpiTest {
    ...
    void registerCustomProvider() {
        McpClientFactory.register("custom_mock", new McpClientProvider() {
            @Override
            public String typeName() { return "custom_mock"; }
            @Override
            public McpClient create(McpServerConfig config) { return new SseClient(config); }
        });
        assertTrue(McpClientFactory.hasProvider("custom_mock"));

        McpServerConfig config = McpServerConfig.builder()
                .serverName("test-custom")
                .serverPath("http://localhost:9999/mcp")
                .clientType("custom_mock")
                .build();
        McpClient client = McpClientFactory.create(config);
        assertNotNull(client);
    }
    ...
    void registerOverridesExisting() {
        ...
        McpClientFactory.register("test_override", new McpClientProvider() {
            @Override
            public String typeName() { return "test_override"; }
            @Override
            public McpClient create(McpServerConfig config) { return new SseClient(config); }
        });
        ...
        McpClientFactory.register("test_override", new McpClientProvider() {
            ...
            public String typeName() { return "test_override"; }
            @Override
            public McpClient create(McpServerConfig config) { return new StdioClient(config); }
        });

        McpServerConfig config = McpServerConfig.builder()
                .serverName("test-override")
                .serverPath("/bin/test")
                .clientType("test_override")
                .build();
        McpClient client = McpClientFactory.create(config);
    ...
    void createOpenApiClient() {
        McpServerConfig config = McpServerConfig.builder()
                .serverName("test-openapi")
                .serverPath("http://localhost:8080/openapi.json")
                .clientType("openapi")
                .build();
        McpClient client = McpClientFactory.create(config);
        assertNotNull(client);
    ...
    void createStreamableHttpClient() {
        McpServerConfig config = McpServerConfig.builder()
                .serverName("test-streamable-http")
                .serverPath("http://localhost:8080/mcp")
    ...
    void createPlaywrightClient() {
        McpServerConfig config = McpServerConfig.builder()
                ...
                .serverPath("http://localhost:8080/playwright")
    ...
    void registerProviderThatReadsConfig() {
        McpClientFactory.register("config_aware", new McpClientProvider() {
            @Override
            public String typeName() { return "config_aware"; }
            @Override
        ...
        McpServerConfig config = McpServerConfig.builder()
                .serverName("test-config-aware")
                .serverPath("http://localhost:9999/mcp")
                .clientType("config_aware")
                .params(Map.of("custom_key", "custom_value"))
                .build();
        McpClient client = McpClientFactory.create(config);
        assertInstanceOf(SseClient.class, client);
    }
