...
class HarnessPermissionCompatibilityTest {
    ...
    void permissionEngineShouldEvaluateAllowAskDeny() {
        ...
        Map.Entry<PermissionLevel, String> ask = engine.evaluateGlobalPolicyDirectly("read_file", Map.of("path", "a.txt"));
        Map.Entry<PermissionLevel, String> deny = engine.evaluateGlobalPolicyDirectly("write_file", Map.of("path", "a.txt"));
    ...
    void toolPermissionHostShouldExposeWorkspaceAndPersistAllowRule() {
        ToolPermissionHost host = ToolPermissionHost.builder()
                .resolveWorkspaceDir(() -> Path.of("/tmp/workspace"))
                .permissionYamlPath(Path.of("/tmp/permissions.yaml"))
                .getPermissionsSnapshot(HarnessPermissionCompatibilityTest::permissions)
                .build();
    ...
    void permissionRailShouldAllowDenyAndInterruptByPolicy() {
        ...
        ToolCallInputs readInputs = ToolCallInputs.builder()
                .toolCall(ToolCall.builder().id("tc1").name("read_file").arguments("{}").build())
                .toolName("read_file")
                .toolArgs(Map.of("path", "a.txt"))
                .build();
        AgentCallbackContext readCtx = AgentCallbackContext.builder().inputs(readInputs).extra(new LinkedHashMap<>()).build();
        ...
        ToolCallInputs denyInputs = ToolCallInputs.builder()
                .toolCall(ToolCall.builder().id("tc2").name("write_file").arguments("{}").build())
                .toolName("write_file")
                .toolArgs(Map.of("path", "a.txt"))
                .build();
        AgentCallbackContext denyCtx = AgentCallbackContext.builder().inputs(denyInputs).extra(new LinkedHashMap<>()).build();
        rail.beforeToolCall(denyCtx);
        assertThat(denyCtx.getExtra()).containsEntry("_skip_tool", Boolean.TRUE);

        ToolCallInputs allowInputs = ToolCallInputs.builder()
                .toolCall(ToolCall.builder().id("tc3").name("list_files").arguments("{}").build())
                .toolName("list_files")
                .toolArgs(Map.of())
                .build();
        AgentCallbackContext allowCtx = AgentCallbackContext.builder().inputs(allowInputs).extra(new LinkedHashMap<>()).build();
