...
class HarnessBrowserMoveCompatibilityTest {
    ...
    void actionControllerShouldRegisterListAndRunActions() {
        ...
        Map<String, Object> result = controller.runAction("MYACTION", "session-1", "request-1", Map.of("source", "test"));
    ...
    void browserRuntimeAndHelperToolsShouldDelegate() throws Exception {
        ...
        BrowserAgentRuntime runtime = new BrowserAgentRuntime(
                ...
                new BrowserService("openai", "test-key", "https://example.invalid/v1", "test-model", cfg,
                        BrowserRunGuardrails.builder().maxSteps(3).maxFailures(1).timeoutS(30).retryOnce(false).build()) {
                    @Override
                    protected java.util.Map<String, Object> runTaskOnce(String task, String sessionId, String requestId, Integer timeoutS) {
        ...
        Map<String, Object> taskResult = runtime.runBrowserTask("Submit onboarding form", "session-1", "request-1", 120);
        runtime.controller().registerExampleActions();
        runtime.controller().bindRuntimeRunner(runtime::runBrowserTask);
        Map<String, Object> custom = runtime.runCustomAction("browser_task", "s4", "r4", Map.of("task", "go to example.com", "timeout_s", 7));
        ...
        assertThat(functionTools)
                .hasSize(5)
                .allSatisfy(tool -> assertThat(tool.getCard().getId()).startsWith("browser-owner."));
        assertThat(actionTool.invoke(Map.of(
                "action", "browser_task",
                "session_id", "s5",
                "request_id", "r5",
                "params", Map.of("task", "open example.org", "timeout_s", 8)
