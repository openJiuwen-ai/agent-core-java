...
class GitHubCliCompatibilityTest {
    ...
    void ensureReadyShouldReportAuthenticatedGhWhenAlreadyInstalled() {
        ...
        GitHubCli cli = new GitHubCli(
                command -> "gh".equals(command) ? "/usr/bin/gh" : "",
                (command, timeoutMillis) -> new GitHubCli.CommandResult(0, "", ""),
                "Linux",
                false
        );
    ...
    void ensureReadyShouldInstallGhAndPromptForLoginWhenAuthMissing() {
        ...
        assertThat(runCalls).containsExactly(
                List.of("brew", "install", "gh"),
    ...
    void ensureReadyShouldFallBackWhenGhInstallFails() {
        ...
        GitHubCli cli = new GitHubCli(
                command -> "",
                (command, timeoutMillis) -> new GitHubCli.CommandResult(1, "", "permission denied"),
                "Linux",
                false,
                List.of(new GitHubCli.InstallCommand(List.of("apt-get", "install", "-y", "gh"), "apt-get install -y gh"))
        );
    ...
    void installCommandsShouldUsePlatformPackageManagersAndNonInteractiveSudo() {
        ...
        assertThat(commands).extracting(GitHubCli.InstallCommand::label)
                .contains("apt-get install -y gh", "pacman -S --noconfirm github-cli");
        assertThat(commands).extracting(GitHubCli.InstallCommand::command)
                .contains(
