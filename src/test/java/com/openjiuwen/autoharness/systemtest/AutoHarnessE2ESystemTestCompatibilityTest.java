...
class AutoHarnessE2ESystemTestCompatibilityTest {
    ...
    void resolveApiConfigShouldMirrorPythonE2EEnvironmentAliases() {
        Map<String, String> loaded = Map.of(
                "API_BASE", "https://from-file.example",
                "API_KEY", "file-key",
                "MODEL_NAME", "file-model",
                "MODEL_PROVIDER", "file-provider"
        );
        Map<String, String> env = Map.of(
                "OPENJIUWEN_API_BASE", "https://openjiuwen.example",
                "OPENJIUWEN_API_KEY", "openjiuwen-key",
                "OPENJIUWEN_MODEL", "openjiuwen-model",
                "OPENJIUWEN_PROVIDER", "openjiuwen-provider",
                "MODEL_TIMEOUT", "77"
        );
    ...
    void resolveApiConfigShouldPreferCanonicalApiEnvironmentOverAliases() {
        Map<String, String> resolved = AutoHarnessE2ESystemTest.resolveApiConfig(
                Map.of(),
                Map.of(
                        "API_KEY", "canonical-key",
                        "OPENJIUWEN_API_KEY", "alias-key",
                        "MODEL_NAME", "canonical-model",
                        "OPENJIUWEN_MODEL", "alias-model"
                )
        );
    ...
    void resolveApiConfigShouldUseSettingsAndPythonModelDefaults() {
        Map<String, String> resolved = AutoHarnessE2ESystemTest.resolveApiConfig(
                ...
                Map.of(),
                Map.of(
                        "apiBase", "https://settings.example",
                        "apiKey", "settings-key"
                )
        );
    ...
    void resolveApiConfigShouldLetEnvironmentOverrideSettingsLikePythonE2E() {
        Map<String, String> resolved = AutoHarnessE2ESystemTest.resolveApiConfig(
                Map.of(),
                Map.of("OPENJIUWEN_PROVIDER", "env-provider"),
                Map.of("provider", "settings-provider")
        );
