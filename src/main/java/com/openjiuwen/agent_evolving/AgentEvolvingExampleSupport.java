 ...
 *
 * <p>The current Java baseline exposes dataset/evaluator/trainer building
 * blocks, but not the Python online/offline RL launcher stack.</p>
 */
public final class AgentEvolvingExampleSupport {
    ...
    public static CaseLoader calculatorCaseLoader() {
        return new CaseLoader(List.of(
                Case.builder()
                        .caseId("calc-1")
                        .inputs(Map.of("query", "2+2", "ground_truth", "4"))
                        .label(Map.of("answer", "4"))
                        .build()
        ));
    }
    ...
    public static CaseLoader nl2sqlCaseLoader() {
        return new CaseLoader(List.of(
                Case.builder()
                        .caseId("sql-1")
                        .inputs(Map.of(
                                "query", "Database: database/concert_singer\n\nQuestion: count all singers",
                                "ground_truth", "SELECT count(*) FROM singer"
                        ))
                        .label(Map.of("answer", "SELECT count(*) FROM singer"))
                        .build()
        ));
    }
