 ...
 *
 * <p>Mirrors Python's {@code ShellOperation} in {@code local/shell_operation.py}.
 */
...
public class LocalShellOperation extends BaseShellOperation {
  ...
  public ExecuteCmdResult executeCmd(
    ...
    try {
      ...
      String encoding =
          options != null && options.containsKey("encoding")
  ...
  public Iterator<ExecuteCmdStreamResult> executeCmdStream(
    ...
    try {
      ...
      int chunkSize =
          options != null && options.containsKey("chunk_size")
              ? (Integer) options.get("chunk_size")
              : 1024;
      String encoding =
          options != null && options.containsKey("encoding")
              ? (String) options.get("encoding")
  ...
  public ExecuteCmdBackgroundResult executeCmdBackground(
    ...
    try {
      ...
      if (!checkAllowlist(command)) {
        return buildCmdBackgroundErrorResult(
            "command not allowed by allowlist",
      ...
      if (grace > 0) {
        ...
        if (!process.isAlive()) {
          ...
          return buildCmdBackgroundErrorResult(
              "background command exited early with code " + exitCode,
      ...
      return ExecuteCmdBackgroundResult.builder()
          .code(StatusCode.SUCCESS.getCode())
  ...
  private static long graceMillis(double grace) {
    return Math.max(
        1L,
        BigDecimal.valueOf(grace).movePointRight(3).setScale(0, RoundingMode.HALF_UP).longValue());
  }
  ...
  private boolean checkAllowlist(String command) {
    ...
    return allowlist.stream()
        .anyMatch(
            isAllowed -> cmdPrefix.equals(isAllowed) || cmdPrefix.endsWith(separator + isAllowed));
  }
  ...
  private Path resolveCwd(String cwd) {
    LocalWorkConfig config =
        getRunConfig() instanceof LocalWorkConfig localConfig ? localConfig : null;
    String workDirVal = config != null ? config.getWorkDir() : null;
    Path baseDir =
        workDirVal != null
            ? Path.of(workDirVal).toAbsolutePath()
            : Path.of(CwdContext.getCwd()).toAbsolutePath();
  ...
  private ExecuteCmdStreamResult buildCmdStreamErrorResult(
      String errorMsg, ExecuteCmdChunkData data) {
    if (data != null && (data.getExitCode() == null || data.getExitCode() == 0)) {
      data.setExitCode(-1);
    }
    return BaseResult.buildOperationErrorResult(
  ...
  private ExecuteCmdBackgroundResult buildCmdBackgroundErrorResult(
      String errorMsg, ExecuteCmdBackgroundData data) {
    return BaseResult.buildOperationErrorResult(
        StatusCode.SYS_OPERATION_SHELL_EXECUTION_ERROR,
  ...
  private ExecuteCmdStreamResult transformCmdStreamEvent(StreamEvent event, int chunkIndex) {
    return switch (event.getType()) {
      ...
      case ERROR ->
          buildCmdStreamErrorResult(
              "execution receive error: " + event.getDataAsString(),
              ExecuteCmdChunkData.builder().chunkIndex(chunkIndex).exitCode(-1).build());
      case EXIT -> {
