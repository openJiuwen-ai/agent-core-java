...
/**
 * SiliconFlow API客户端，支持GPT模型和OpenAI兼容服务。
 * 对应 Python: agent-core/openjiuwen/core/foundation/llm/model_clients/siliconflow_model_client.py
 * 
 ...
 * </pre>
 */
public class SiliconFlowModelClient extends BaseModelClient implements AutoCloseable {

    private static final Logger log = LoggerFactory.getLogger(SiliconFlowModelClient.class);
    ...
    public void close() {
        synchronized (clientLock) {
            if (sharedHttpClient != null) {
                ...
                log.debug("SiliconFlowModelClient closed");
            }
        }
    }
     ...
     * 构建并清理请求参数
     */
    private Map<String, Object> buildAndSanitizeParams(
            Object messages,
            List<?> tools,
            Double temperature,
            Double topP,
        ...
        Map<String, Object> params = buildRequestParams(
                messages, tools, temperature, topP, model, stop, maxTokens, stream, kwargs
        );
        // 清理messages中的tool_calls
        @SuppressWarnings("unchecked")
        List<Map<String, Object>> messagesList = (List<Map<String, Object>>) params.get("messages");
        params.put("messages", sanitizeToolCalls(messagesList));
        return params;
    }

    /**
     * 清理tool_calls，只保留OpenAI标准字段
     * 保留: id, type, index, function.name, function.arguments
     * 强制type为"function"
     */
    @SuppressWarnings("unchecked")
    private List<Map<String, Object>> sanitizeToolCalls(List<Map<String, Object>> messages) {
        ...
        for (Map<String, Object> msg : messages) {
            ...
            for (Object tcObj : toolCalls) {
                ...
                Map<String, Object> cleanedTc = new LinkedHashMap<>();
                cleanedTc.put("id", tc.getOrDefault("id", ""));
                cleanedTc.put("type", "function");
                cleanedTc.put("index", tc.get("index"));
                cleanedTc.put("function", Map.of(
                        "name", func.getOrDefault("name", ""),
                        "arguments", func.getOrDefault("arguments", "")
                ));
                cleaned.add(cleanedTc);
            }
    ...
    public CompletableFuture<AssistantMessage> invoke(
        ...
        return CompletableFuture.supplyAsync(() -> {
            ...
            // 验证URL
            UrlUtils.checkUrlIsValid(apiUrl);

            log.info("SiliconFlow API request ready for model: {}", params.get("model"));
            ...
            try {
                ...
                try (Response response = client.newCall(request).execute()) {
                    if (!response.isSuccessful()) {
                        ...
                        throw new JiuWenBaseException(
                                ...
                                "SiliconFlow API error: " + response.code() + " - " + errorBody
                        );
                    }

                    String responseBody = response.body() != null ? response.body().string() : "";
                    log.info("SiliconFlow API response received for model: {}", params.get("model"));
                ...
                log.error("SiliconFlow API invoke error", e);
                throw new JiuWenBaseException(
                        StatusCode.MODEL_CALL_FAILED.getCode(),
                        "SiliconFlow API async invoke error: " + e.getMessage()
                );
            }
        });
    }
        ...
        try {
            ...
            factory.newEventSource(request, new EventSourceListener() {
                ...
                public void onFailure(EventSource eventSource, Throwable t, Response response) {
                    ...
                    log.error("SiliconFlow stream error", t);
                }
            ...
            public boolean hasNext() {
                ...
                try {
                    ...
                    if (error[0] != null) {
                        throw new JiuWenBaseException(
                                StatusCode.MODEL_CALL_FAILED.getCode(),
                                "SiliconFlow stream error: " + error[0].getMessage()
                        );
                    }
                    return false;
     ...
     */
    @SuppressWarnings("unchecked")
    public AssistantMessageChunk parseStreamChunk(String data) {
        try {
            // 处理[DONE]标记
            ...
            // 跳过空块
            if ((content == null || content.isEmpty()) && reasoningContent == null && toolCalls.isEmpty()) {
                return null;
            }
    ...
    /**
     * 解析SSE格式的字节数据（用于兼容Python测试）
     */
    public AssistantMessageChunk parseStreamChunk(byte[] chunk) {
