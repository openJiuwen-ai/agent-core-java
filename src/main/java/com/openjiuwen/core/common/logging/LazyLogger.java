 ...
 *
 * <p>Mirrors Python's {@code LazyLogger} in
 * {@code openjiuwen/core/common/logging/__init__.py}.</p>
 *
 * <p>Logger is only initialized when actually used, improving startup performance.
 * Suitable for static/module-level logger definitions.</p>
 */
public class LazyLogger implements LoggerProtocol {
    ...
    private LoggerProtocol getDelegate() {
        LoggerProtocol current = getter.get();
        if (delegate != current) {
            synchronized (this) {
                current = getter.get();
                if (delegate != current) {
