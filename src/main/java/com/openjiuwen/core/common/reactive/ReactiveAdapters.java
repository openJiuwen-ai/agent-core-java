...
public final class ReactiveAdapters {
     ...
     */
    public static <T> Flux<T> fromCallableIterator(
            java.util.concurrent.Callable<? extends Iterator<T>> source, Runnable cleanup) {
        return Mono.fromCallable(source::call)
                .subscribeOn(Schedulers.boundedElastic())
     ...
     */
    public static <T> Flux<T> fromAutoCloseableIterator(
            java.util.concurrent.Callable<? extends Iterator<T>> source) {
    ...
    public static <T> Flux<T> fromAutoCloseableIterator(
        ...
        return Flux.usingWhen(
                Mono.fromCallable(source::call)
                        .subscribeOn(Schedulers.boundedElastic()),
                it -> fromIterator(it),
                it -> closeAndCleanup(it, cleanup));
    }

    private static Mono<Void> closeAndCleanup(Iterator<?> iterator, Runnable cleanup) {
        Mono<Void> close = iterator instanceof AutoCloseable closeable
                ? Mono.fromCallable(() -> {
                    closeable.close();
                    return Boolean.TRUE;
