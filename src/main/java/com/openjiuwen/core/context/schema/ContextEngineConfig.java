 ...
 *
 * <p>Mirrors Python's {@code ContextEngineConfig} from {@code context_engine/schema/config.py}.
 */
@Data
@Builder
@NoArgsConstructor
...
public class ContextEngineConfig {
  ...
  /**
   * Maximum number of messages retained in the context message buffer. {@code null} means
   * unlimited. Must be > 0 if set.
   */
  ...
  @Builder.Default private boolean enableKvCacheRelease = false;

  /** Whether to enable the reload tool that can re-inject offloaded messages. */
  @Builder.Default private boolean enableReload = false;
