 ...
 *
 * <p>Mirrors Python's {@code ContextProcessorStateRecorder}.
 */
public class ContextProcessorStateRecorder {
  ...
  public void emit(SessionModelContext context, ContextCompressionState state) {
    ...
    if (session == null) {
      Loggers.CONTEXT_ENGINE.debug(
          "context compression state stream skipped: no session writer session_id={} context_id={}"
              + " op={}",
          sessionId,
          contextId,
          state.getOperationId());
    ...
    try {
      session.writeStream(
          new OutputSchema(ContextCompressionState.CONTEXT_COMPRESSION_STATE_TYPE, 0, state));
  ...
  public ContextCompressionState buildState(ContextProcessorStateInput input) {
    ContextCompressionMetric before =
        buildMetric(input.beforeMessages(), input.contextMax(), input.startedAt());
    ContextCompressionMetric after =
        input.afterMessages() != null
    ...
    return ContextCompressionState.builder()
        ...
        .status(input.status())
        .phase(input.phase())
        .processor(input.processor() != null ? input.processor().processorType() : "")
        .model(resolveModelName(input.processor(), input.trigger(), input.isForce()))
        .before(before)
        .after(after)
        .statistic(buildStatistic(statisticMessages))
        .saved(saved)
        .durationMs(
            input.endedAt() != null ? (int) ((input.endedAt() - input.startedAt()) * 1000) : null)
        .contextMax(input.contextMax())
        .summary(
            buildSummary(
                input.status(), before, after, saved, input.reason(), input.messagesToModify()))
        .error(input.error())
        .build();
  }
  ...
  private void record(ContextCompressionState state) {
    history.add(
        Map.of(
            "type", state.getType(),
            "operation_id", state.getOperationId(),
            "status", state.getStatus(),
            "phase", state.getPhase(),
            "processor", state.getProcessor(),
  ...
  private int approximateTokens(List<BaseMessage> messages) {
    ...
    for (BaseMessage message : messages) {
      totalChars +=
          String.valueOf(message.getContent() != null ? message.getContent() : "").length();
    }
  ...
  private int countMessageForStatistic(BaseMessage message) {
    ...
    try {
      return tokenCounter.count(
          String.valueOf(message.getContent() != null ? message.getContent() : ""));
    } catch (IllegalArgumentException | IllegalStateException ignored) {
      return 0;
    }
  }
  ...
  private static ContextCompressionSaved buildSaved(
    ...
    float savedPercent =
        before.getTokens() > 0
            ? Math.round((savedTokens * 1000.0f / before.getTokens())) / 10.0f
            : 0.0f;
  ...
  private static String buildSummary(
    ...
    if ("failed".equals(status)) {
      return "Context processor failed; context remains ~"
          + compactNumber(before.getTokens())
          + " tokens";
    }
    ...
    if ("noop".equals(status)) {
      return "Context unchanged at ~"
          + compactNumber(after.getTokens())
          + " tokens "
          + "(saved "
    ...
    String modified =
        messagesToModify != null && !messagesToModify.isEmpty()
  ...
  private static String resolveModelName(
      ContextProcessor processor, String trigger, boolean isForced) {
    if (processor == null) {
      return "";
    }
    ...
    String modelFromNested =
        readStringProperty(readProperty(config, "model"), "modelName", "model");
