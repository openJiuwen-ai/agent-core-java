...
public class VerifyStage extends TaskStage {
    ...
    public StageResult run(BaseExecutionContext ctx) {
        ...
        return StageResult.builder()
                .status("failed")
                .error("verify stage did not return StageResult")
                .build();
    }
    ...
    public List<Object> stream(BaseExecutionContext ctx) {
        if (!(ctx instanceof TaskContext taskContext)) {
            return List.of(StageResult.builder()
                    .status("failed")
                    .error("verify requires TaskContext")
                    .build());
        }
        ...
        if (!ci.isPassed()) {
            ...
            if (!fix.isOk()) {
                ...
                events.add(StageResult.builder()
                        .status("failed")
                        .artifacts(Map.of(
                                "verify_report", VerifyReportArtifact.builder()
                                        .ciResult(ciResult)
                                        .isReverted(true)
                                        .error(errorLog)
                                        .build(),
                        ...
                        .messages(concat(messages, "修复失败，回滚变更"))
                        .error(errorLog)
                        .build());
                return events;
            }
    ...
    public static List<String> iterCiGateMessages(Map<String, Object> ciResult, String prefix) {
        ...
        String summary = String.join(", ", gates.stream()
                .map(gate -> valueOrDefault(String.valueOf(gate.get("name")), "unknown")
        ...
        try {
            FixLoopResult result = ctx.getOrchestrator().getFixLoop().run(
                    ...
                    () -> {
                        ...
                        CIGateResult ci = ctx.getOrchestrator().getCiGate().run("all");
                        Map<String, Object> ciResult = ciResult(ci);
                        String query = "任务描述: " + value(ctx.getTask().getDescription()) + "\n\n"
                                + "代码变更:\n" + truncate(diff, 5000) + "\n\n"
            ...
            FixLoopResult result = FixLoopResult.builder()
                    .errorLog(List.of(ex.getMessage() == null ? "" : ex.getMessage()))
                    .build();
    ...
    public static Map<String, Object> ciResult(CIGateResult ci) {
        ...
        if (ci != null && ci.getGates() != null && !ci.getGates().isEmpty()) {
            ...
            for (String output : ci.getGateOutputs()) {
                ...
                gates.add(new LinkedHashMap<>(Map.of(
                        "name", valueOrDefault(name, "unknown"),
                        "isPassed", isPassed,
                        "output", detail
                )));
            }
