...
public class SecurityRail extends DeepAgentRail {
  ...
  public SecurityRail(List<String> immutableFiles, List<String> highImpactPrefixes) {
    ...
    this.highImpactPrefixes =
        highImpactPrefixes == null ? List.of() : List.copyOf(highImpactPrefixes);
  }
  ...
  public void beforeToolCall(AgentCallbackContext ctx) {
    if (!(ctx.getInputs() instanceof ToolCallInputs inputs)
        || !WRITE_TOOLS.contains(inputs.getToolName())) {
      return;
    }
    ...
    if (matchesAny(filePath, immutableFiles)) {
      EditSafetyRail.rejectTool(
          ctx,
          inputs,
          "File '"
              + filePath
              + "' is immutable and must not be modified. "
