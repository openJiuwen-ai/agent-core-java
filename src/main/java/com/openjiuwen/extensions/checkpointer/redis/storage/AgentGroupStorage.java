...
/**
 * Mirrors Python's {@code AgentGroupStorage} in
 * {@code openjiuwen.extensions.checkpointer.redis.storage}.
 *
 * <p>Stores only the shared/global state for an agent-team session.</p>
 */
    ...
    public CompletableFuture<Void> clear(String teamId, String sessionId) {
        try {
            redisStore.batchDelete(List.of(
                    ...
                    key(sessionId, teamId, STATE_BLOBS)), null);
            return CompletableFuture.completedFuture(null);
        } catch (Throwable throwable) {
            return CompletableFuture.failedFuture(wrapFailure(throwable));
        }
