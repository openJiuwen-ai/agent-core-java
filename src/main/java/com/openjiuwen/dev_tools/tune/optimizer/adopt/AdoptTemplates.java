...
public final class AdoptTemplates {
    ...
    public static final String DEEP_OUTPUT_ANALYSIS_SYSTEM_PROMPT = """
            ...
            Your task is to explain **why** the workflow's actual output deviates from the expected output, using only the provided external knowledge.
    ...
    public static final String GRADIENT_GENERATE_SYSTEM_PROMPT = """
            ...
            Your task is to analyze the current prompt, job of the node, input, actual output, and the expected output for a single case.
    ...
    public static final String GRADIENT_GENERATE_USER_PROMPT = """
            ...
            <OBJECT>
            Based on the above, modify the <CURRENT_PROMPT> so that the node will produce <EXPECTED_OUTPUT> instead of <CURRENT_OUTPUT>.
            </OBJECT>
            """;
    ...
    public static final String GRADIENT_REDUCE_SYSTEM_PROMPT = """
            ...
            1. Synthesize a set of individual "Reasoning:" outputs from different cases into summary of failure patterns.
    ...
    public static String buildNodeIoString(List<?> inputList, List<?> outputList) {
        ...
        for (int i = 0; i < length; i++) {
            sb.append(String.format(
                    ...
                    i, safeParseJson(inputList.get(i)), i,
                    i, safeParseJson(outputList.get(i)), i
            ));
        }
        return sb.toString();
    }
