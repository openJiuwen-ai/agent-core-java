...
/**
 * ADOPT prompt templates and node I/O formatting helpers.
 *
 * <p>Mirrors Python's {@code openjiuwen.dev_tools.tune.optimizer.adopt.utils} in
 * {@code openjiuwen/dev_tools/tune/optimizer/adopt/utils.py}.</p>
 */
public final class AdoptTemplates {
    private static final ObjectMapper MAPPER = new ObjectMapper();

    public static final PromptTemplate OUTPUT_CHANGE_SYSTEM_PROMPT = system("""
    ...
    public static final PromptTemplate OUTPUT_CHANGE_USER_PROMPT = user("""
    ...
    public static final PromptTemplate DEEP_OUTPUT_ANALYSIS_SYSTEM_PROMPT = system("""
    ...
    public static final PromptTemplate DEEP_OUTPUT_ANALYSIS_USER_PROMPT = user("""
    ...
    public static final PromptTemplate EXPECTED_OUTPUT_SYSTEM_PROMPT = system("""
    ...
    public static final PromptTemplate EXPECTED_OUTPUT_USER_PROMPT = user("""
    ...
    public static final PromptTemplate GRADIENT_GENERATE_SYSTEM_PROMPT = system("""
    ...
    public static final PromptTemplate GRADIENT_GENERATE_USER_PROMPT = user("""
    ...
    public static final PromptTemplate GRADIENT_REDUCE_SYSTEM_PROMPT = system("""
    ...
    public static final PromptTemplate GRADIENT_REDUCE_USER_PROMPT = user("""
    ...
    public static final PromptTemplate PROMPT_UPDATE_SYSTEM_PROMPT = system("""
    ...
    public static final PromptTemplate PROMPT_UPDATE_USER_PROMPT = user("""
    ...
    public static final PromptTemplate CONCLUDE_AGENT_SYSTEM_PROMPT = system("""
            ## Role
            You are a master of LLM-workflow analysis, capable of precisely identifying the task of the entire workflow, the responsibility of every LLM node, and the dependency between each node鈥檚 output and the final result.
            ...
            Clarify how the quality of each node鈥檚 output influences the end-to-end final result
    ...
    public static final PromptTemplate CONCLUDE_AGENT_USER_PROMPT = system("""
    ...
    public static final PromptTemplate CONCLUDE_NODE_SYSTEM_PROMPT = system("""
            ## Role
            ...
            3. Combine the case data to reason about **how the LLM node鈥檚 output correlates with the final output**.
            4. Consider how **changes in the LLM node鈥檚 output** might **positively or negatively impact** the final workflow result.
            ...
            2. Based on the good cases, what is the **correlation** between this LLM node鈥檚 output and the final output?
            3. Based on the good cases, how might **changes in the LLM node鈥檚 output** positively or negatively affect the final workflow output?
    ...
    public static final PromptTemplate CONCLUDE_NODE_USER_PROMPT = system("""
    ...
    public static String buildNodeIoString(List<?> inputList, List<?> outputList) {
        ...
        if (length == 1) {
            return "- <NODE_INPUT> " + safeParseJson(inputList.get(0)) + " </NODE_INPUT>\n"
                    + "- <CURRENT_WRONG_NODE_OUTPUT> " + safeParseJson(outputList.get(0))
                    + " </CURRENT_WRONG_NODE_OUTPUT>";
        }
        StringBuilder builder = new StringBuilder();
        for (int index = 0; index < Math.min(inputList.size(), outputList.size()); index += 1) {
            if (index > 0) {
                builder.append('\n');
            }
            builder.append("- <NODE_INPUT_")
                    .append(index)
                    .append("> ")
                    .append(safeParseJson(inputList.get(index)))
                    .append(" </NODE_INPUT_")
                    .append(index)
                    .append(">\n")
                    .append("- <CURRENT_WRONG_NODE_OUTPUT_")
                    .append(index)
                    .append("> ")
                    .append(safeParseJson(outputList.get(index)))
                    .append(" </CURRENT_WRONG_NODE_OUTPUT_")
                    .append(index)
                    .append(">");
        }
    ...
    public static String safeParseJson(Object value) {
    ...
    private static PromptTemplate system(String content) {
        return template(new SystemMessage("\n" + content));
    }

    private static PromptTemplate user(String content) {
        return template(new UserMessage("\n" + content));
    }

    private static PromptTemplate template(BaseMessage message) {
