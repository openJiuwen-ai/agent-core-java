...
final class DiscoveryMetadataProviders {
    ...
    static final class ListSkillMetadataProvider implements ToolMetadataProvider {
        ...
        public String getDescription(String language) {
            return text(language, "列出可用技能或为当前任务选择相关技能。",
    ...
    static final class LoadToolsMetadataProvider implements ToolMetadataProvider {
        ...
        @Override
        public String getDescription(String language) {
            return text(language, "将选定的真实工具加载到当前 session 可见工具集合中。",
                    ...
                    .objectSchema(
                            ToolSchemaSupport.properties(new Object[]{"tool_names",
                                    Map.of("type", "array", "items", Map.of("type", "string"), "description",
                                            text(language, "要在当前 session 中可见的工具名称列表",
                                                    "Names of tools to make visible for the current session")),
                                    "replace",
                                    ToolSchemaSupport.property("boolean", text(language, "如果为 true，替换当前可见工具集，否则合并",
    ...
    static final class SearchToolsMetadataProvider implements ToolMetadataProvider {
        ...
        @Override
        public String getDescription(String language) {
            return text(language, "根据能力、名称、描述或参数提示搜索候选工具。仅用于发现，不会直接调用工具。",
            ...
            return ToolSchemaSupport.objectSchema(
                    ToolSchemaSupport.properties(new Object[]{"query",
                            ToolSchemaSupport.property("string",
                                    text(language, "搜索候选工具的查询文本", "Search query for finding relevant candidate tools")),
                            "limit",
                            ToolSchemaSupport.property("integer",
                                    text(language, "返回候选工具的最大数量", "Maximum number of candidate tools to return")),
                            "detail_level",
                            ToolSchemaSupport.property("integer",
                                    text(language, "1=name+描述, 2=+参数摘要, 3=+完整参数",
    ...
    static final class SkillToolMetadataProvider implements ToolMetadataProvider {
        ...
        @Override
        public String getDescription(String language) {
            return text(language, "使用此工具查看特定技能的内容", "Use this tool to view the skill contents of a certain skill");
        }
        ...
        public Map<String, Object> getInputParams(String language) {
            return ToolSchemaSupport.objectSchema(ToolSchemaSupport.properties(new Object[]{"skill_name",
                    ToolSchemaSupport.property("string", text(language, "技能的名称", "Name of the skill")),
                    "relative_file_path",
                    ToolSchemaSupport.property("string",
                            text(language, "可选。查看技能目录中指定路径下的特定文件。留空则查看主 SKILL.md 文件。",
