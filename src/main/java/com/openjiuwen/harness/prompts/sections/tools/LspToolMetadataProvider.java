    ...
    public Map<String, Object> getInputParams(String language) {
        return ToolSchemaSupport.objectSchema(
                ToolSchemaSupport.properties(new Object[] {
                        "operation", ToolSchemaSupport.enumProperty("string", OPERATIONS, text(language,
                        ...
                        "file_path", ToolSchemaSupport.property("string", text(language,
                                "文件路径（绝对路径或相对于工作区根目录的路径）",
                                "The absolute or relative path to the file")),
                        "line", Map.of(
                                "type", "integer",
                                "minimum", 1,
                                "description", text(language,
                                        "行号（1-indexed，编辑器中显示的行号）",
                        ...
                        "query", ToolSchemaSupport.property("string", text(language,
                                "搜索查询字符串；为空时返回所有可用符号（仅 workspaceSymbol 使用）",
                                "Search query string; when empty, returns all available symbols")),
                        "include_declaration", ToolSchemaSupport.property("boolean", text(language,
                                "为 true 时，结果中包含符号的定义位置（默认 true）",
