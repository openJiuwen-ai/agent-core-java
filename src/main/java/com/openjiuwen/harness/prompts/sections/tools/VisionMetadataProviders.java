...
final class VisionMetadataProviders {
  ...
  static final class ImageOCRMetadataProvider implements ToolMetadataProvider {
    ...
    public String getDescription(String language) {
      return text(
          language,
          "读取图片中的可见文本，适合 OCR、票据文本提取和截图文字识别。",
              ...
              new Object[] {
                "image_path_or_url",
                    ToolSchemaSupport.property(
                        ...
                        text(
                            language,
                            "本地图片路径或公网 http(s) 图片 URL",
                            "Local image path or public http(s) image URL")),
                "prompt",
                    ToolSchemaSupport.property(
                        "string", text(language, "可选，自定义 OCR 提示词", "Optional custom OCR prompt"))
              }),
  ...
  static final class VisualQuestionAnsweringMetadataProvider implements ToolMetadataProvider {
    ...
    public String getDescription(String language) {
      return text(
          language,
          "理解图片内容并回答问题，可选先做 OCR 再结合识别到的文字回答。",
          "Understand an image and answer questions, optionally grounding the answer with OCR"
              + " first.");
    }
      ...
      return ToolSchemaSupport.objectSchema(
          ToolSchemaSupport.properties(
              new Object[] {
                ...
                "question",
                    ToolSchemaSupport.property(
                        "string", text(language, "要询问图片的问题", "Question to ask about the image")),
                "include_ocr",
                    ToolSchemaSupport.property(
                        "boolean",
                        text(
                            language,
                            "是否先执行 OCR 并把结果拼接进问答提示词，默认 true",
                            "Whether to run OCR first and inject the result into the VQA prompt, "
                                + "default true")),
                "ocr_prompt",
                    ToolSchemaSupport.property(
                        "string",
                        text(
                            language,
                            "可选，自定义 OCR 提示词，仅在 include_ocr 为 true 时使用",
  ...
  static final class VideoUnderstandingMetadataProvider implements ToolMetadataProvider {
    ...
    public String getDescription(String language) {
      return text(
          language,
          "理解视频内容并回答用户问题，支持远程视频 URL 或本地视频文件路径。",
          ...
          ToolSchemaSupport.properties(
              new Object[] {
                "query",
                    ToolSchemaSupport.property(
                        "string",
                        text(language, "用户关于视频内容的问题", "User query about the video content")),
                "video_path",
                    ToolSchemaSupport.property(
                        "string",
                ...
                "model",
                    ToolSchemaSupport.property(
                        "string", text(language, "可选，指定模型名称", "Optional model name")),
                "max_tokens",
                    ToolSchemaSupport.property(
                        "integer",
                        text(language, "可选，最大输出 token 数", "Optional maximum output tokens")),
                "temperature",
                    ToolSchemaSupport.property(
                        "number", text(language, "可选，采样温度", "Optional sampling temperature")),
                "timeout_seconds",
                    ToolSchemaSupport.property(
                        "integer", text(language, "可选，请求超时时间（秒）", "Optional timeout in seconds"))
              }),
