...
final class MemoryMetadataProviders {
            ...
            return ToolSchemaSupport.objectSchema(
                    ToolSchemaSupport.properties(new Object[] {
                            "query", ToolSchemaSupport.property("string", text(language,
                                    "检索关键词或问题", "Search query string")),
                            "max_results", ToolSchemaSupport.property("integer", text(language,
                                    "最多返回条数（可选）", "Maximum number of results (optional)")),
                            "min_score", ToolSchemaSupport.property("number", text(language,
                                    "最小相关度阈值（可选）", "Minimum relevance score threshold (optional)")),
                            "session_key", ToolSchemaSupport.property("string", text(language,
                                    "会话键（可选，用于上下文隔离/过滤）",
    ...
    static final class WriteMemoryMetadataProvider implements ToolMetadataProvider {
        ...
        public Map<String, Object> getInputParams(String language) {
            return ToolSchemaSupport.objectSchema(
                    ToolSchemaSupport.properties(new Object[] {
                            "path", pathProperty(language),
                            "content", ToolSchemaSupport.property("string", text(language,
                                    "要写入的内容", "Content to write")),
                            "append", ToolSchemaSupport.property("boolean", text(language,
                                    "是否追加写入（默认 false）", "Append to file instead of overwrite (default false)"))
                    }),
    ...
    static final class EditMemoryMetadataProvider implements ToolMetadataProvider {
        ...
        public String getDescription(String language) {
            return text(language,
                    "在 memory/ 下的记忆文件中做精确字符串替换（old_text → new_text）。",
        ...
        public Map<String, Object> getInputParams(String language) {
            return ToolSchemaSupport.objectSchema(
                    ToolSchemaSupport.properties(new Object[] {
                            "path", pathProperty(language),
                            "old_text", ToolSchemaSupport.property("string", text(language,
                                    "要替换的原始文本", "Original text to isReplace")),
                            "new_text", ToolSchemaSupport.property("string", text(language,
                                    "替换后的新文本", "New replacement text"))
                    }),
    ...
    static final class ReadMemoryMetadataProvider implements ToolMetadataProvider {
        ...
        public String getDescription(String language) {
            return text(language,
                    "按 offset/limit 读取 memory/ 下记忆文件的部分内容（用于分页阅读）。",
    ...
    static final class CodingMemoryReadMetadataProvider implements ToolMetadataProvider {
        ...
        public String getDescription(String language) {
            return text(language,
                    "按 offset/limit 读取 coding_memory/ 下的编码记忆 Markdown 文件。",
    ...
    static final class CodingMemoryWriteMetadataProvider implements ToolMetadataProvider {
        ...
        public String getDescription(String language) {
            return text(language,
                    "写入编码记忆到 coding_memory/ 下的 Markdown 文件；内容必须包含 frontmatter。",
    ...
    static final class CodingMemoryEditMetadataProvider implements ToolMetadataProvider {
        ...
        public String getDescription(String language) {
            return text(language,
                    "在 coding_memory/ 下的编码记忆文件中做精确字符串替换（old_text → new_text）。",
    ...
    private static Map<String, Object> pathWindowSchema(String language, String startField, String countField) {
        return ToolSchemaSupport.objectSchema(
                ToolSchemaSupport.properties(new Object[] {
                        "path", pathProperty(language),
                        startField, ToolSchemaSupport.property("integer", text(language,
                                "起始行号（可选）", "Starting line number (optional)")),
                        countField, ToolSchemaSupport.property("integer", text(language,
                                "读取行数（可选）", "Number of lines to read (optional)"))
                }),
                List.of("path")
        );
    }
    ...
    private static Map<String, Object> pathProperty(String language) {
        return ToolSchemaSupport.property("string", text(language,
                "memory/ 下的目标文件路径（相对路径）", "Target path under memory/ (relative path)"));
    }
