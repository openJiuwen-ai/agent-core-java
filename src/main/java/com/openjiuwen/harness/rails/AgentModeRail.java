...
public class AgentModeRail extends DeepAgentRail {
    ...
    private static final Set<String> HIDDEN_IN_PLAN = Set.of(
            "todo_create",
            "todo_list",
            "todo_modify",
            "sessions_list",
    ...
    private static final Set<String> DEFAULT_PLAN_ALLOWED_TOOLS = Set.of(
            "switch_mode",
            "enter_plan_mode",
            "exit_plan_mode",
            "ask_user",
    ...
    public ToolOutput validateToolCall(AgentMode mode, String toolName, Map<String, Object> toolArgs, Path planPath) {
        ...
        if (HIDDEN_IN_PLAN.contains(toolName)) {
            return ToolOutput.builder()
                    .success(false)
                    .error("[AgentModeRail] tool is hidden in plan mode: " + toolName)
                    .build();
        }
        if (!allowsToolInPlanMode(toolName)) {
            return ToolOutput.builder()
                    .success(false)
                    .error("[AgentModeRail] tool is not available in plan mode: " + toolName)
                    .build();
        }
        if (!allowsWriteTarget(toolName, extractFilePath(toolArgs), planPath)) {
            return ToolOutput.builder()
                    .success(false)
                    .error("[AgentModeRail] write/edit can only target the plan file")
                    .build();
        }
        return ToolOutput.builder().success(true).build();
    }
    ...
    public void beforeToolCall(AgentCallbackContext ctx) {
        ...
        ToolOutput result = validateToolCall(
                owner.getCurrentMode(),
                inputs.getToolName(),
                normalizeArgs(inputs.getToolArgs()),
                owner.getPlanFilePath()
        );
        if (result.isSuccess()) {
            return;
        }
    ...
    private static ToolOutput enterPlanMode(DeepAgent agent, SwitchModeTool switchModeTool, String conversationId) {
        ...
        return ToolOutput.builder()
                .success(true)
                .data(Map.of(
                        "previous_mode", ((Map<?, ?>) switched.getData()).get("previous_mode"),
                        "current_mode", ((Map<?, ?>) switched.getData()).get("current_mode"),
                        "plan_file_path", planPath.toString()
                ))
                .build();
    }
    ...
    private static ToolOutput exitPlanMode(DeepAgent agent, SwitchModeTool switchModeTool) {
        ...
        return ToolOutput.builder()
                ...
                .data(Map.of(
                        "previous_mode", ((Map<?, ?>) switched.getData()).get("previous_mode"),
                        "current_mode", ((Map<?, ?>) switched.getData()).get("current_mode"),
                        "plan_file_path", planPath != null ? planPath.toString() : "",
    ...
    private void registerOwnedTaskTool(DeepAgent agent) {
        if (ownedTaskTool != null
                || agent.getConfig().getSubagents() == null
                || agent.getConfig().getSubagents().isEmpty()) {
            return;
        }
        TaskTool taskTool = new TaskTool(agent);
        ToolCard metadataCard = ToolMetadataRegistry.buildToolCard(
                "task_tool",
                agent.getCard().getId() + ".plan_mode.task_tool",
                agent.getWorkspace().getLanguage()
        );
        ownedTaskTool = new LocalFunction(
                metadataCard,
