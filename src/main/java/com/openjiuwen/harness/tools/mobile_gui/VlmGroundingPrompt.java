...
/**
 * VLM grounding system prompt builder.
 *
 * <p>Mirrors Python's {@code openjiuwen/harness/tools/mobile_gui/vlm_grounding_prompt.py}.</p>
 */
public final class VlmGroundingPrompt {

    private VlmGroundingPrompt() {
    }

    public static String buildVlmGroundingSystemPrompt(MobileGuiRuntimeSettings settings) {
        int scale = settings.getVlmCoordinateScale();
        String skillRules = settings.getSkillConsultMode() == SkillConsultMode.BRANCH
                ? skillRulesBranch()
                : skillRulesInline();
        String parallelRule = settings.getSkillConsultMode() == SkillConsultMode.BRANCH
                ? """
                        6. **One Action Per Step (Strict)**
        ...
        return """
                You are an intelligent Android GUI Agent. Your task is to execute user instructions step by step by observing raw Android screenshots and grounding actions to coordinates.
                ...
                1. **Operate via Coordinates (Function Calling)**
                   ...
                   - The latest observation message states the coordinate range for the screenshot you are seeing. Usually this is normalized to [0, %d], but some models use the sent screenshot's pixel width/height.
                ...
                """.formatted(scale, skillRules.strip(), parallelRule.strip());
    }

    private static String skillRulesInline() {
        return """
                3. **Read Skills First; Re-search When Stuck**
                   - You MUST read skill first whenever relevant SKILL.md content is available in context, read and follow it before improvising.
                   - If the UI stops progressing, do not repeat the same coordinate blindly. Re-check skill context, choose a different visible target, scroll, go back, or wait.
                   - Use skill_tool for skill markdown (`SKILL.md` and sibling docs). Figures in SKILL are not attached automatically - when you need reference pixels, call `read_file` on the image path under `skills/<skill_name>/...` (paths appear like `![](images/foo.png)`). Optionally pass `caption` with the markdown alt text.
                   - After calling the skill_tool, read at least 2 image in the skill (as list of tool calls) which you think is relevant to the current task.
                   - Skill tool text may start with a short reminder that embedded images are links only, not screenshots; the multimodal lead for a `read_file` of a skill image will state that it is documentation, not the live device screen.
                """;
    }

    private static String skillRulesBranch() {
        return """
                3. **Read Skills First; Re-search When Stuck**
