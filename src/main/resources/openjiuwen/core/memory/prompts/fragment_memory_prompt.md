...
- 规则：
  ...
  - 若用户表述中涉及时间推理时，执行时间推理，时间推理规则如下：
    ...
    - "下周"：将【当前所在周】中"（即XX.XX～XX.XX）"的两个数字日期各+7天，得到下一个完整周
    - "上周"：将【当前所在周】中"（即XX.XX～XX.XX）"的两个数字日期各-7天，得到上一个完整周
    - "上个月"：上一个完整自然月（1日～月末）。禁止将当天日期往前推30天
    - "下个月"：下一个完整自然月（1日～月末）
    - "X天前/X天后"：从【对话发生时间】精确加减对应天数
    - 仅给出范围词无具体日期时，输出完整日期范围
...
- 规则：
  ...
  - 对于【情景记忆信息】分类：主语全部改为"用户"，**必须**将所有相对时间（下周、上周、上个月、昨天等）按照Step 3的推理结果替换为绝对日期，**禁止**在输出中保留任何相对时间词。时间范围用"-"连接（如"用户在2026年1月6日-2026年1月12日之间去北京出差"）
