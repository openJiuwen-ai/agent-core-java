...
> **注意**：本文档基于 Python 版 openJiuwen 的 Context Evolver 扩展编写。Java 版对应实现请参考 `com.openjiuwen.extensions.contextevolver` 包及 `documents/zh/2.开发指南/API文档/com.openjiuwen.extensions/` 下的 Java API 文档。以下代码示例中的包引用已统一替换为 `com.openjiuwen.*`，部分 Python 异步调用模式（`await`/`asyncio`）在 Java 中对应同步 API 或 `CompletableFuture`。
...
java -jar examples.context_evolver.quickstart
```
...
```java
import com.openjiuwen.extensions.context_evolver.service.task_memory_service import TaskMemoryService
import com.openjiuwen.extensions.context_evolver.context_evolving_react_agent import ContextEvolvingReActAgent, create_memory_agent_config
from com.openjiuwen.core.single_agent.schema.agent_card import AgentCard
import com.openjiuwen.extensions.context_evolver.service.task_memory_service import AddMemoryRequest
...
- `com.openjiuwen.core.retrieval.embedding.api_embedding` 用于嵌入
...
# 运行特定测试文件
java -jar pytest tests/unit_tests/extensions/context_evolver/test_retrieve_flow.py -v
java -jar pytest tests/unit_tests/extensions/context_evolver/test_summary_flow.py -v
java -jar pytest tests/unit_tests/extensions/context_evolver/test_file_connector.py -v
java -jar pytest tests/unit_tests/extensions/context_evolver/test_quickstart.py -v
```
