...
### 架构组件
- **Transport（传输层）**: 负责智能体间消息传递（支持 `inprocess`/本地线程、远程通信）
...
```java
import yaml
import com.openjiuwen.agent_teams import TeamAgentSpec
...
| `spawn_mode` | `inprocess`/本地线程（同进程）或 `process`（子进程） |
...
// Java: 使用 CompletableFuture 或线程池异步消费
