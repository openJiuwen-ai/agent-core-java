...
本栏目整理 Java 版已经具备、但通常需要跨多个子系统一起理解的能力：检索、记忆、上下文、安全护栏、执行与恢复、Session、Skills、系统操作、提示词优化、组件扩展以及 agent evolving。每一页都只以 Java 当前公开 API、源码和测试覆盖范围为准。
...
| 页面 | 关注点 | 主要依据 | 说明 |
| --- | --- | --- | --- |
| [知识检索](知识检索.md) | 知识库、索引、检索与重排 | `com.openjiuwen.core.retrieval` | 以 Java retrieval API 和测试覆盖为准。 |
| [记忆引擎](记忆引擎.md) | 长期记忆、作用域配置、记忆检索 | `com.openjiuwen.core.memory` | 结合 memory 根包与管理子包理解。 |
| [上下文引擎](上下文引擎.md) | `ContextEngine`、上下文窗口、处理器与状态 | `com.openjiuwen.core.context` | 覆盖上下文生命周期与配置入口。 |
...
| 页面 | 关注点 | 主要依据 | 说明 |
| --- | --- | --- | --- |
| [Agent Skills](<Agent Skills.md>) | 本地技能、远程技能、技能注册与提示词拼装 | `singleagent.skills` | 聚焦 Java 当前技能系统。 |
| [系统操作](系统操作.md) | `SysOperation`、本地/沙箱模式、结果模型 | `com.openjiuwen.core.sysop` | 解释系统操作门面和运行模式。 |
| [技能与系统操作](技能与系统操作.md) | 技能与 sysop 的组合关系 | `singleagent.skills`、`sysop` | 强调工具适配与运行边界。 |
...
| [ReActAgent演化训练](ReactAgent强化学习.md) | 演化训练、评估与优化流程 | `ReActAgentEvolve` | 聚焦 Java 当前可运行的演化训练主线。 |
...
> 当前 Java 0.1.14 仓库没有独立的 `examples/` 目录。Python 文档中的示例语义已经尽量迁移到对应专题页；继续下钻时请以 Java 源码、API 文档和测试入口为准。
