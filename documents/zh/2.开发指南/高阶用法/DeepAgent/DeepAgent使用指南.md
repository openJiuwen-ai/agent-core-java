...
| 项 | 说明 |
| --- | --- |
| 默认 max | `max(40, CPU 核数 × 8)`（I/O 型 workload，随 CPU 缩放） |
| 默认 queue | `128` |
...
9. **工具异常终态配置**：`shouldFailTaskOnToolError` 默认 `true`，工具执行抛出异常时默认将任务推进到 `FAILED` 终态（触发「失败释放额度」）；如需让工具异常作为可恢复错误交回 LLM 继续对话，显式设为 `false`。
