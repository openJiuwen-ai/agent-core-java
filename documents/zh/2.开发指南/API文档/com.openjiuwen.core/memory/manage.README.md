...
`com.openjiuwen.memory.manage` 汇总长期记忆引擎的管理层能力，按职责拆分为 `index`、`model`、`search`、`update` 四个子包，分别负责写入协调、底层数据模型、检索参数与增量更新判定。
...
| [`model`](./manage/model.README.md) | 定义 KV/SQL/向量存储适配器、数据模型与枚举类型。 |
