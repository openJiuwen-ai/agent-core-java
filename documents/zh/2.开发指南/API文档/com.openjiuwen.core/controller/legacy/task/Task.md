...
| 字段 | 类型 | 默认�?| 说明 |
|---|---|---|---|
| `agentId` | `String` | `null` | 所�?agent�?|
| `taskId` | `String` | 空字符串 | 任务 ID�?|
| `taskType` | `TaskType` | `UNDEFINED` | 任务类型�?|
| `description` | `String` | `null` | 任务描述�?|
| `status` | `TaskStatus` | `PENDING` | 任务状态�?|
| `metadata` | `Map<String, Object>` | 空映�?| 附加元数据�?|
| `input` | `TaskInput` | 新实�?| 任务输入�?|
| `result` | `TaskResult` | `null` | 任务结果�?|
| `dependencies` | `List<TaskDependency>` | 空列�?| 依赖列表�?|
| `dependents` | `Set<String>` | 空集�?| 被哪些任务依赖�?|
| `parentTaskId` | `String` | `null` | 父任�?ID�?|
| `childTaskIds` | `Set<String>` | 空集�?| 子任�?ID 集合�?|
| `groupId` | `String` | `null` | 所�?group�?|
| `level` | `int` | `0` | 层级深度�?|
...
- 该类型依�?Lombok 生成访问器和 builder�?
- `IntentDetectionController` 和旧�?reasoner/planner 主要围绕这一任务模型协作�?
