...
| `outputs` | `List<ControllerOutputChunk>` | 空列�?| 任务输出块列表�?|
| `status` | `TaskStatus` | `UNKNOWN` | 当前状态�?|
| `parentTaskId` | `String` | `null` | 父任�?ID�?|
| `contextId` | `String` | `null` | 关联上下�?ID�?|
...
| 签名 | 说明 |
|---|---|
| `Task()` | 初始化默认优先级、空输出列表�?`UNKNOWN` 状态�?|
...
- `sessionId`、`taskId` �?`taskType` 不能为空白字符串�?
- `priority` 不能小于 `0`�?
...
- `TaskManager` 对外返回的几乎都�?`copy()` 结果，因此调用方修改任务后需要显式调�?`updateTask()` 才会写回管理器�?
