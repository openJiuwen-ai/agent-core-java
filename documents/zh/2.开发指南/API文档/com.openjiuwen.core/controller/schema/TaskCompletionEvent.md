...
| 字段 | 类型 | 默认值 | 说明 |
|---|---|---|---|
| `taskResult` | `List<DataFrame>` | 空列表 | 任务完成后的输出数据。 |
...
| 签名 | 说明 |
|---|---|
| `TaskCompletionEvent()` | 创建空完成事件，并固定 `eventType = TASK_COMPLETION`。 |
| `TaskCompletionEvent(List<DataFrame> taskResult, Task task)` | 使用给定结果和任务对象构造完成事件。 |

## 说明

- `TaskScheduler.publishTaskEvent()` 在收到 `task_completion` payload 时会构造该事件并重新送回 `EventQueue`。
- `EventHandlerWithIntentRecognition.handleTaskCompletion()` 会把 `taskResult` 以 `result` 键写入会话流。
