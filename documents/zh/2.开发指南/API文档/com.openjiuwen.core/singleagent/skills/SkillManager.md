...
| 签名 | 说明 |
|---|---|
| `public SkillManager(String sysOperationId)` | 使用给定�?`sysOperationId` 初始化技能管理器�?|
...
| `public void register(String skillPath)` | 以默�?`sessionId = null`、`overwrite = false` 注册单个路径�?|
...
- `registerRoot(...)` 会优先把传入路径本身当作技能文件解析；若解析失败且路径是目录，则扫描其直接子目录中�?`Skill.md` �?`SKILL.md`�?
- `loadDescription(...)` 只从 YAML front matter �?`description:` 字段提取技能描述�?
- `SkillManagerTest` 验证�?`SKILL.md`/`Skill.md` 注册、重复注册保留原条目、注销和计数逻辑�?
