...
package com.openjiuwen.memory.graph.extraction.prompts.entity_extraction;
...
import com.openjiuwen.memory.graph.extraction.RelationDef;
...
public final class ExtractionPromptLanguageCn {
  ...
  public static void registerLanguage() {
    ...
    ExtractionPromptLanguageBase.RELATION_FORMAT.put(
        LANGUAGE_CODE, "{name}（<{lhs}>-[{name}]-<{rhs}>）：{description}");
    ...
    MultilingualBaseModel.registerMultilingualDescription(
        LANGUAGE_CODE,
        Map.ofEntries(
            Map.entry("{{[ent_def_name]}}", "新提取实体的名字"),
            Map.entry("{{[ent_def_type]}}", "新提取实体的类型id，需要在提供的实体类型中"),
            Map.entry("{{[ent_ext_list]}}", "新提取的实体列表"),
            Map.entry("{{[ent_summary]}}", "实体相关的重要信息，500字以内的简要摘要"),
            Map.entry("{{[ent_attributes]}}", "实体的属性值"),
            Map.entry("{{[ent_dupe_name]}}", "现有实体的名字"),
            Map.entry("{{[ent_dupe_id]}}", "现有实体的ID"),
            Map.entry("{{[ent_dupe_id_list]}}", "与现有实体重复的实体ID列表"),
            ...
            Map.entry("{{[rel_fact]}}", "关于实体联系的事实"),
            Map.entry("{{[rel_name]}}", "该实体联系的名称"),
            Map.entry("{{[rel_source_id]}}", "主体的实体ID"),
            Map.entry("{{[rel_target_id]}}", "客体的实体ID"),
            Map.entry("{{[rel_ext_list]}}", "新提取的关系列表"),
            Map.entry("{{[rel_filter_list]}}", "与给定实体可能相关的事实ID列表"),
            ...
            Map.entry("{{[rel_dupe_id_list]}}", "需要与新增关系融合的已有关系ID列表"),
            Map.entry("{{[year]}}", "年"),
            Map.entry("{{[month]}}", "月"),
            Map.entry("{{[day]}}", "日"),
            Map.entry("{{[hour]}}", "小时"),
            Map.entry("{{[minute]}}", "分钟"),
            Map.entry("{{[second]}}", "秒"),
            Map.entry("{{[tz_name]}}", "时区名"),
            Map.entry("{{[tz_offset]}}", "相对于UTC标准时的时差（用+HH:MM格式）"),
            Map.entry("{{[tz_reason]}}", "为什么可能是这个时区"),
            Map.entry("{{[tz_list]}}", "可能的时区列表"),
            Map.entry(":", "：")));
