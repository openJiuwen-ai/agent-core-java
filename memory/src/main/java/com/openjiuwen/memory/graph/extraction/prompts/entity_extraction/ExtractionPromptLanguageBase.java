...
public final class ExtractionPromptLanguageBase {
  ...
  public static String formatSchemaInfo(
    ...
    MultilingualBaseModel instance;
    try {
      instance = outputModel.getDeclaredConstructor().newInstance();
    } catch (Exception exception) {
      return "";
    }
    MultilingualBaseModel.ReadableSchema readable = instance.readableSchema(language);
    StringBuilder builder = new StringBuilder(SCHEMA_INFO_HEADER);
    if (!readable.refs().isEmpty()) {
      builder.append("# ").append(REF_JSON_OBJECT_DEF.get(language)).append("\n");
      for (Map.Entry<String, Object> entry : readable.refs().entrySet()) {
        builder
            .append("## ")
            .append(entry.getKey())
            .append("\n```json\n")
