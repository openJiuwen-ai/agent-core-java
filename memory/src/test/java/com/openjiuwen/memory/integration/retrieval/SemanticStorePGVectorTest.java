...
package com.openjiuwen.memory.integration.retrieval;
...
class SemanticStorePGVectorTest {
    ...
    void createCollectionUsesExplicitBootstrapWithoutInsert() throws Exception {
        ...
        TestPGVectorStore backend = new TestPGVectorStore(
                new VectorStoreConfig("pgvector", "memory_db", "memory_fragments", "cosine"), dataSource, "vector",
                Map.of("vector_field", "embedding"));
        SemanticStore semanticStore = new SemanticStore(new PGVectorStore(backend));
    ...
    void addDocsBootstrapsAndWritesRows() throws Exception {
        ...
        TestPGVectorStore backend = new TestPGVectorStore(
                new VectorStoreConfig("pgvector", "memory_db", "memory_fragments", "cosine"), dataSource, "vector",
                Map.of("vector_field", "embedding"));
        SemanticStore semanticStore = new SemanticStore(new PGVectorStore(backend), new FixedEmbedding());
    ...
    private static class TestPGVectorStore extends com.openjiuwen.retrieval.vector_store.PGVectorStore {
