...
public class ProcessSpawnHandle implements SpawnHandle {
    ...
    public boolean shutdown(Long timeoutMillis) {
        Double timeoutSeconds = timeoutMillis == null
                ? null
                : BigDecimal.valueOf(timeoutMillis).divide(BigDecimal.valueOf(1_000L)).doubleValue();
        return delegate.shutdown(timeoutSeconds).toCompletableFuture().join();
    }
    ...
    public void forceKill() {
        delegate.forceKill().toCompletableFuture().join();
    }
    ...
    public int waitForCompletion() {
        return delegate.waitForCompletion().toCompletableFuture().join();
    }
    ...
    public boolean isHealthCheckRunning() {
        return delegate.isAlive() && !delegate.isHealthy();
    }
}
