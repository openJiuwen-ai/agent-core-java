...
public class SpawnManager {
    ...
    /**
     * Block on a completion stage and unwrap its result, re-throwing the underlying cause
     * (or wrapping non-runtime exceptions in a CompletionException).
     *
     ...
     * @since 0.1.7
     */
    private static <T> T awaitStage(java.util.concurrent.CompletionStage<T> stage) {
        try {
    ...
    public SpawnHandle spawnTeammate(
        ...
        SpawnConfig effectiveConfig =
                spawnConfig != null
                        ? spawnConfig
                        : new SpawnConfig(50.0D, 10.0D, 30.0D);
        SpawnHandle handle;
        if ("inprocess".equals(teamAgent.getSpec().getSpawnMode())) {
            ...
            Object spawned = awaitStage(Runner.spawnAgent(
                    ...
                    null,
                    effectiveConfig));
            if (!(spawned instanceof SpawnedProcessHandle)) {
                String actualType =
                        spawned == null ? "null" : spawned.getClass().getSimpleName();
                throw new IllegalStateException(
                        "Runner.spawnAgent must return SpawnedProcessHandle, got " + actualType);
            }
            handle = new ProcessSpawnHandle((SpawnedProcessHandle) spawned);
        }
        registerHandle(
