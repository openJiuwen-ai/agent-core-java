...
public class WorkflowRuntimeSession extends Vertex.VertexSession
    ...
    public WorkflowRuntimeSession(String workflowId, BaseSession parent, String sessionId,
        ...
        this.state = bindSessionState(state, subGraph);
    ...
    /**
     * Replaces the session tracer sink.
     *
     * @param tracer tracer sink, or {@code null} to clear
     */
    public void setTracer(Vertex.VertexTraceSink tracer) {
        this.tracer = tracer;
    }
    ...
    private WorkflowRuntimeState bindSessionState(WorkflowCommitState state, boolean isSubGraph) {
        if (isSubGraph) {
            return WorkflowRuntimeState.from(state, this.parentId, resolveStateNodeId(true));
        }
        if (state instanceof WorkflowRuntimeState runtimeState) {
            return runtimeState;
        }
        return WorkflowRuntimeState.from(state, this.parentId, resolveStateNodeId(false));
    }

    private String resolveStateNodeId(boolean isSubGraph) {
        if (isSubGraph && !this.executableId.isBlank()) {
            return this.executableId;
        }
        if (!this.parentId.isBlank() && !this.executableId.isBlank()) {
            return this.executableId;
        }
        return WorkflowRuntimeState.DEFAULT_NODE_ID;
    }
