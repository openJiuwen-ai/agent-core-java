...
public class WorkflowRuntimeState extends WorkflowCommitState
    ...
    public static WorkflowRuntimeState from(WorkflowCommitState state, String parentId, String nodeId) {
        String resolvedParentId = parentId != null ? parentId : "";
        String resolvedNodeId = nodeId != null && !nodeId.isBlank() ? nodeId : DEFAULT_NODE_ID;
        if (state instanceof WorkflowRuntimeState runtimeState) {
            return runtimeState.createNodeState(resolvedNodeId, resolvedParentId);
        }
        WorkflowRuntimeState runtimeState = create(resolvedParentId, resolvedNodeId);
        if (state != null) {
    ...
    public Map<String, Object> getInputs(Object schema) {
        ...
        if (originalSchema != resolvedSchema) {
            Object prefixedInputs = super.getInputs(originalSchema);
            inputs = mergeMissingValues(inputs, prefixedInputs);
        }
