...
public class MessageRouter {
    ...
    public CompletableFuture<Object> routeP2pMessage(MessageEnvelope envelope) {
        ...
        try {
            Object session = resolveDispatchSession(envelope.getSessionId(), envelope.getRecipient());
            return runtime.dispatchToAgent(envelope.getRecipient(), envelope.getMessage(), session)
                    .toCompletableFuture();
    ...
    private CompletableFuture<Void> invokeSubscriber(String subscriber, MessageEnvelope envelope) {
        ...
        Object session = resolveDispatchSession(envelope.getSessionId(), subscriber);
        return runtime.dispatchToAgent(subscriber, envelope.getMessage(), session)
    ...
    private Object resolveDispatchSession(String sessionId, String agentId) {
        Object session = buildAgentSession(sessionId, agentId);
        if (session != null) {
            return session;
        }
        // No bound team session: fall back to the raw session id for dispatch.
