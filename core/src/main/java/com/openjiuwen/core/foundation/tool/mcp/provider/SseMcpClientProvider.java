...
public final class SseMcpClientProvider implements McpClientProvider {
    ...
    public McpClient create(McpServerConfig config) {
        return new SyncSseClient(new SseClient(config));
    }

    /**
     * Public adapter so reflective MCP registration can invoke connect/listTools.
     *
     * @since 0.1.18
     */
    public static final class SyncSseClient implements McpClient {
        private final SseClient sseClient;

        SyncSseClient(SseClient sseClient) {
