...
import com.openjiuwen.extensions.context_evolver.retrieve.task.reme.RerankMemoryOp;
import com.openjiuwen.extensions.context_evolver.retrieve.task.reme.RewriteMemoryOp;
...
public class TaskMemoryService {
    ...
    private BaseOp createRetrieveFlow() {
        return switch (retrievalAlgorithm) {
            ...
            case "ReMe" -> createReMeRetrieveFlow(
                configInt("TOPK_RETRIEVAL", 10),
                configBoolean("LLM_RERANK", true),
                configInt("TOPK_RERANK", 5),
                configBoolean("LLM_REWRITE", true)
            );
            case "RefCon", "DivCon" -> createReMeRetrieveFlow(10, true, 5, true);
            default -> new RecallMemoryOp();
        };
    }

    private SequentialOp createReMeRetrieveFlow(
            int topkRetrieval,
            boolean llmRerank,
            int topkRerank,
            boolean llmRewrite) {
        return new SequentialOp(
            new com.openjiuwen.extensions.context_evolver.retrieve.task.reme.RecallMemoryOp(topkRetrieval),
            new RerankMemoryOp(llmRerank, topkRerank),
            new RewriteMemoryOp(llmRewrite)
        );
    }

    private BaseOp createSummaryFlow() {
        return switch (summaryAlgorithm) {
            case "ACE" -> createAceSummaryFlow();
            case "ReasoningBank" -> createReasoningBankSummaryFlow();
            case "ReMe", "RefCon", "DivCon" -> createReMeSummaryFlow();
            default -> throw new IllegalArgumentException(
                "Invalid summary algorithm '" + summaryAlgorithm
                    + "'. Must be one of: ACE, ReasoningBank, ReMe, RefCon, DivCon."
            );
        };
    }

    private BaseOp createAceSummaryFlow() {
        boolean useGroundTruth = configBoolean("USE_GROUNDTRUTH", false);
        int maxPlaybookSize = configInt("MAX_PLAYBOOK_SIZE", 50);
        SequentialOp flow = new SequentialOp(
            new LoadPlaybookOp(),
            new ReflectOp(useGroundTruth),
            new CurateOp(),
            new ApplyDeltaOp(maxPlaybookSize)
        );
        return appendPersist(flow, "ACE");
    }

    private BaseOp createReasoningBankSummaryFlow() {
        SequentialOp flow = new SequentialOp(
            new ReasoningBankSummarizeMemoryOp(),
            new com.openjiuwen.extensions.context_evolver.summary.task.reasoning_bank.UpdateVectorStoreOp()
        );
        return appendPersist(flow, "ReasoningBank");
    }

    private BaseOp createReMeSummaryFlow() {
        SequentialOp flow = new SequentialOp(
            ...
            new com.openjiuwen.extensions.context_evolver.summary.task.reme.UpdateVectorStoreOp()
        );
        return appendPersist(flow, "ReMe");
    }

    private BaseOp appendPersist(BaseOp flow, String algorithm) {
        BaseOp persistOp = createPersistOp(algorithm);
        if (persistOp == null) {
            return flow;
        }
        return flow.then(persistOp);
    }

    private BaseOp createPersistOp(String algorithm) {
        if (persistType == null) {
            return null;
        }
        return switch (algorithm) {
            case "ReasoningBank" ->
                new com.openjiuwen.extensions.context_evolver.summary.task.reasoning_bank.PersistMemoryOp(
                    persistType, persistPath, milvusHost, milvusPort, milvusCollection);
            case "ReMe" ->
                new com.openjiuwen.extensions.context_evolver.summary.task.reme.PersistMemoryOp(
                    persistType, persistPath, milvusHost, milvusPort, milvusCollection);
            default ->
                new com.openjiuwen.extensions.context_evolver.summary.task.ace.PersistMemoryOp(
                    persistType, persistPath, milvusHost, milvusPort, milvusCollection);
        };
    }
