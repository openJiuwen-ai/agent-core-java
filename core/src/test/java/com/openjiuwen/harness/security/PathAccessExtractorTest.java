...
/**
 * Mirrors the 830 {@code files/PathAccessExtractorTest}, adapted to the flat
 * {@code harness.security} package. Absolute-path assertions use {@code endsWith}
 * because literal {@code /etc/...} inputs resolve against the current drive on
 * Windows.
 */
class PathAccessExtractorTest {
    ...
    class ExtractNative {
        ...
        void writeFile_filePath_extractsWrite() {
            ...
            assertThat(posix(out.get(0).getPath())).endsWith("/etc/hosts");
        ...
        void relativePath_resolvedAgainstWorkspace() {
            ...
            assertThat(out).hasSize(1);
            assertThat(posix(out.get(0).getPath())).endsWith("/work/a/b.txt");
        }
    ...
    class ExtractLegacy {
        ...
        void cp_extractsReadThenWrite() {
            ...
            assertThat(posix(out.get(0).getPath())).endsWith("/a");
            assertThat(out.get(0).getAction()).isEqualTo(FileGuardAction.READ);
            assertThat(posix(out.get(1).getPath())).endsWith("/b");
